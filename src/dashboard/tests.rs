...
fn fixture(value: &Value, kind: SourceKind, name: &str) -> Result<(tempfile::NamedTempFile, SourceConfig)> {
    let mut file = tempfile::NamedTempFile::new()?;
    file.write_all(serde_json::to_string(value)?.as_bytes())?;
    let source = SourceConfig {
        name: name.to_owned(), kind, path: file.path().to_path_buf(), enabled: true, optional: false,
    };
    Ok((file, source))
}
...
fn aliases_and_dedup_keep_distinct_billing_providers_and_orgs() -> Result<()> {
    ...
    let (first_file, first) = fixture(&json!({"anthropic":[oauth.clone(), organization],"google-antigravity":oauth}), SourceKind::Omp, "first")?;
    let (second_file, second) = fixture(&json!({"claude":{"type":"oauth","access":"other-token","refresh":"other-refresh","email":"same@example.test","accountId":"account","orgId":"personal"}}), SourceKind::Omp, "second")?;
    let config = DashboardConfig { sources: vec![first, second], ..DashboardConfig::default() };
    let dashboard = Dashboard::load(&config)?;
    assert_eq!(dashboard.accounts.len(), 3);
    let personal = dashboard.accounts.iter().find(|account| account.sources.len() == 2)
        .ok_or_else(|| eyre::eyre!("missing merged account"))?;
    assert_eq!(personal.provider, Provider::Claude);
    assert!(dashboard.accounts.iter().any(|account| account.provider == Provider::Antigravity));
...
fn exclusion_uses_all_duplicate_metadata_and_source_kind_before_discovery() -> Result<()> {
    let (first_file, first) = fixture(&json!({"anthropic":{"type":"oauth","access":"same-synthetic-access","refresh":"synthetic-refresh","accountId":"id"}}), SourceKind::Omp, "old")?;
    let (second_file, second) = fixture(&json!({"anthropic":{"type":"oauth","access":"same-synthetic-access","refresh":"synthetic-refresh","accountId":"id","email":"excluded@example.test"}}), SourceKind::Omp, "new")?;
    let mut config = DashboardConfig { sources: vec![first, second], ..DashboardConfig::default() };
    config.exclude.accounts.push(AccountExclusion { email: "excluded@example.test".to_owned(), provider: "codex".to_owned() });
    let dashboard = Dashboard::load(&config)?;
    assert_eq!(dashboard.accounts.len(), 1);
    assert_eq!(dashboard.accounts[0].email.as_deref(), Some("excluded@example.test"));
    ...
    config.sources.push(SourceConfig { name: "custom-missing".to_owned(), kind: SourceKind::Omp, path: first_file.path().join("missing"), enabled: true, optional: false });
...
fn api_keys_dedup_by_provider_key_and_masks_never_expose_short_keys() -> Result<()> {
    ...
    let (first_file, first) = fixture(&json!({"openrouter":{"type":"api_key","key":key},"anthropic":{"type":"api_key","key":key}}), SourceKind::Omp, "first")?;
    let (second_file, second) = fixture(&json!({"openrouter":{"type":"api_key","key":key}}), SourceKind::Omp, "second")?;
    let mut config = DashboardConfig { sources: vec![first, second], ..DashboardConfig::default() };
    let dashboard = Dashboard::load(&config)?;
...
fn account_id_and_cross_provider_email_exclusions_are_independent() -> Result<()> {
    ...
    }), SourceKind::Omp, "fixture")?;
    let mut config = DashboardConfig { sources: vec![source], ..DashboardConfig::default() };
...
impl FixtureServer {
    fn start() -> Result<Self> {
        ...
        let worker = std::thread::spawn(move || {
                ...
                match listener.accept() {
                    Ok((mut socket, _)) => {
                        ...
                        connections.push(std::thread::spawn(move || {
                            ...
                            let body = if failed { "{\"error\":\"MUST-NOT-LEAK\"}" } else { "{\"data\":{\"usage\":12.5,\"limit\":50,\"is_management_key\":false}}" };
            ...
            for connection in connections { let _ = connection.join(); }
        });
        Ok(Self { endpoint, requests, maximum, stop, worker: Some(worker) })
    }
}
...
async fn visible_refresh_is_bounded_single_flight_and_failure_isolated() -> Result<()> {
    ...
    let (file, source) = fixture(&json!({"openrouter":[
    ...
    let config = DashboardConfig { sources: vec![source], concurrency: 2, ..DashboardConfig::default() };
    ...
    let visible: Vec<_> = dashboard.accounts.iter().take(3).map(|account| account.id.clone()).collect();
    ...
    assert_eq!(first.iter().filter(|snapshot| snapshot.usage.is_some()).count(), 2);
    assert_eq!(second.iter().filter(|snapshot| snapshot.error.is_some()).count(), 1);
