use super::{
    authorized, nonnegative, percent_metric, request_json, text, timestamp, HttpError,
...
fn add_bucket(usage: &mut AccountUsage, bucket: &Value, group: Option<&str>) {
    ...
    if percent.is_some() || resets_at.is_some() {
        usage
            .windows
            .push(percent_metric(name.clone(), percent, resets_at));
    }
    if percent.is_none() {
        if let Some(balance) = bucket.get("remainingAmount").and_then(nonnegative) {
            usage.credits.push(CreditBalance {
                name: format!("{name} remaining"),
                balance,
                unit: "quota units (unspecified)".to_owned(),
                expires_at: None,
            });
        }
    }
...
fn counter<'a>(quota: &'a Value, model: &'a Value, model_id: &'a str) -> &'a str {
    let provider = text(quota, "modelProvider")
        .or_else(|| text(quota, "apiProvider"))
...
fn parse_models(payload: &Value) -> Result<AccountUsage, HttpError> {
    ...
    for (model_id, model) in models {
        for quota in model_quotas(model) {
            ...
            if percent.is_none() && reset.is_none() {
                continue;
            }
            let counter = counter(quota.value, model, model_id);
            let window = text(quota.value, "windowLabel")
                .or_else(|| text(quota.value, "windowId"))
                .or(quota.window)
                .map_or("Quota", window_name);
            let tier = quota
                .tier
                .or_else(|| text(quota.value, "tier"))
                .unwrap_or("");
            let name = if tier.is_empty() {
                format!("{counter}: {window}")
            } else {
                format!("{counter}: {window} ({tier})")
            };
            ...
            let metric = metrics
                .entry(key)
                .or_insert_with(|| percent_metric(name, percent, reset));
    ...
    Ok(AccountUsage {
        windows: metrics.into_values().collect(),
...
mod tests {
    ...
    fn summary_keeps_shared_third_party_bucket_single_and_unknown_units_unknown() -> eyre::Result<()>
    {
        ...
        assert!(usage
            .credits
            .first()
            .is_some_and(|credit| (credit.balance - 42.0).abs() < f64::EPSILON));
        Ok(())
    }
    ...
    fn legacy_reset_only_is_unknown_and_backend_windows_are_not_combined() -> eyre::Result<()> {
        ...
        let google = usage
            .windows
            .iter()
            .find(|metric| metric.name.starts_with("Google"))
