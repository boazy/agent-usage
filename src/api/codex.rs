use super::{
    authorized, millis, nonnegative, now_ms, percent_metric, request_json, timestamp, HttpError,
};
use crate::auth::AuthRecord;
use crate::dashboard::{AccountUsage, CreditBalance};
use crate::time::now_millis;
use crate::usage::{
    collect_items, parse_reset_credits, parse_usage_payload, window_label, ParsedUsage, UsageStatus,
};
...
fn normalize(parsed: &ParsedUsage, details: Option<&Value>) -> Result<AccountUsage, HttpError> {
    let windows = collect_items(parsed, now_millis())
        .into_iter()
        .map(|item| {
            ...
            percent_metric(
                format!(
                    "{} {}{status}",
                    item.meter,
                    window_label(item.limit_window_seconds)
                ),
                item.used_percent,
    ...
    let detail_credits = details.and_then(parse_reset_credits).map(|mut credits| {
        credits.retain(|credit| {
            credit
                ...
                .and_then(millis)
                .zip(now)
                .is_none_or(|(expiry, now)| expiry > now)
        });
        credits
    });
    let mut available = details
        .and_then(|value| value.get("available_count"))
        .and_then(Value::as_u64)
        .or_else(|| {
            detail_credits
                .as_ref()
                .map_or(parsed.reset_credits_available, |credits| {
...
mod tests {
    ...
    fn live_bank_count_overrides_stale_usage_and_redeemed_entries() -> eyre::Result<()> {
        ...
        assert!(empty
            .credits
            .iter()
            .all(|credit| credit.balance.abs() < f64::EPSILON));
        Ok(())
    }
}
