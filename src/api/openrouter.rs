use super::{
    authorized, nonnegative, number, request_json, spending_metric, text, timestamp, HttpError,
};
...
pub(super) async fn fetch(
    ...
    if management {
        ...
        usage.credits.push(CreditBalance {
            name: "Account-wide credits".to_owned(),
            balance: total - used,
            unit: "USD".to_owned(),
            expires_at: None,
        });
...
fn parse(payload: &Value) -> Result<AccountUsage, HttpError> {
    ...
    for (field, name) in [
        ...
        if let Some(used) = data.get(field).and_then(nonnegative) {
            usage
                .windows
                .push(spending_metric(name.to_owned(), Some(used), None, "USD"));
        }
    }
    if let Some(limit) = data.get("limit").and_then(nonnegative) {
        ...
        usage
            .windows
            .push(spending_metric(name.to_owned(), used, Some(limit), "USD"));
        if let Some(balance) = remaining {
            usage.credits.push(CreditBalance {
                name: "Key budget remaining".to_owned(),
                balance,
                unit: "USD".to_owned(),
...
mod tests {
    ...
    fn budget_uses_remaining_not_lifetime_spend_or_external_byok() -> eyre::Result<()> {
        ...
        let budget = usage
            .windows
            .iter()
            .find(|metric| metric.limit.is_some())
            .ok_or_else(|| eyre::eyre!("missing budget"))?;
        assert!(budget
            .used
            .is_some_and(|used| (used - 25.0).abs() < f64::EPSILON));
        assert!(budget
            .used_percent
            .is_some_and(|used| (used - 25.0).abs() < f64::EPSILON));
        ...
        assert!(unlimited
            .windows
            .iter()
            .all(|metric| metric.limit.is_none() && metric.used_percent.is_none()));
